...
class RegistrationCubit extends Cubit<RegistrationState> {
  ...
  void goToStepTwo() {
    if (!state.data.isStepOneValid) {
      emit(state.copyWith(error: 'Vui lòng điền đầy đủ thông tin và kiểm tra mật khẩu'));
      return;
    }
    ...
    emit(state.copyWith(
      currentStep: RegistrationStep.stepTwo,
      error: null,
    ));
  }
  ...
  void goBackToStepOne() {
    emit(state.copyWith(
      currentStep: RegistrationStep.stepOne,
      error: null,
    ));
  }
  ...
  Future<void> submitRegistration() async {
    if (!state.data.isStepTwoValidForRole(role)) {
      String errorMessage = role == 'PASSENGER' 
